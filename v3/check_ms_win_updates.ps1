    ...
    try {
        For ( $i = 0; $i -lt $Args.count; $i++ ) {
            ...
            switch -regex -casesensitive ($CurrentArg) {
                ...
                "^(-M|--Method)$" {
                    if ($value -match "(^PSWindowsUpdate$)|(^UpdateSearcher$)|(^WMI$)|(^Custom$)") {
                        $Struct.Method = $value
                    } else {
                ...
                "(-S|--Severity)$" {
                    if ($value -match "(^Critical$)|(^Important$)|(^Moderate$)|(^Low$)") {
                        $Struct.Severity = $value
                    } else {
                        throw "Criticity `"$value`" does not meet regex requirements. The value must be one of these : Critical, Important, Moderate, Low "
                    }
                    $i++
                }
                "^(-h|--Help)$" {
...
Function Search-WithPSWindowsUpdate {
    ...
    try {
        switch ($Struct.Severity) {
            "Critical" {
                Try {
                    $Struct.NumberOfUpdates = ((Get-WUList -Notcategory 'Driver' -severity "Critical" -WarningAction SilentlyContinue) | Measure-Object).count
            
                } 
                Catch {
                    Write-Log Verbose Info "Failed to count updates with Get-WUList."
                }
            }
            "Important" {
                Try {
                    $Struct.NumberOfUpdates = ((Get-WUList -Notcategory 'Driver' -severity "Critical" -WarningAction SilentlyContinue) | Measure-Object).count
                    $Struct.NumberOfUpdates += ((Get-WUList -Notcategory 'Driver' -severity "Important" -WarningAction SilentlyContinue) | Measure-Object).count
            ...
            "Moderate" {
                Try {
                    $Struct.NumberOfUpdates = ((Get-WUList -Notcategory 'Driver' -severity "Critical" -WarningAction SilentlyContinue) | Measure-Object).count
                    $Struct.NumberOfUpdates += ((Get-WUList -Notcategory 'Driver' -severity "Important" -WarningAction SilentlyContinue) | Measure-Object).count
                    $Struct.NumberOfUpdates += ((Get-WUList -Notcategory 'Driver' -severity "Moderate" -WarningAction SilentlyContinue) | Measure-Object).count
                } 
                Catch {
                    Write-Log Verbose Info "Failed to count updates with Get-WUList."
                }
        ...
        $Struct.LastSuccesTime = [datetime](Get-WUHistory -last 100 | Measure-Object Date -Maximum).Maximum
...
Function Search-WithCustom {
    ...
    if (Test-Path $LastSuccessTimeFolder) {
        ...
        if ($LastSuccessTimeValue) {
            ...
            Write-Log Verbose Info "LastSuccessTime not found in the registry."
        }
    }
    
    if (-not $Struct.LastSuccesTime) {
        Try {
            $LastUpdate = Get-WmiObject -Class "win32_quickfixengineering" | Select-Object -Property "Description", "HotfixID", @{label="InstalledOn";e={[DateTime]::Parse($_.psbase.properties["installedon"].value,$([System.Globalization.CultureInfo]::GetCultureInfo("en-US")))}} | Sort-Object -Property InstalledOn | Select-Object InstalledOn -Last 1
            $Struct.LastSuccesTime = $LastUpdate.InstalledOn
        }
        Catch { 
            Write-Log Verbose Info "Something went wrong while querying WMI or date conversion."
        }
    }
    
    if ($Struct.LastSuccesTime) {
        Write-Log Verbose Info "La dernière mise à jour a été installée le : $($Struct.LastSuccesTime)"
    } else {
        Write-Log Verbose Info "Impossible de déterminer la date de la dernière mise à jour."
    }
##########################
switch ($Struct.Severity) {
    "Critical" {
...
###########################
    ...
    Catch {
        Write-Log Verbose Info "Failed to determine reboot status."
    }
    ...
    Try {
        ...
        if($Struct.NumberOfUpdates -ge 1 -and $CriticalLimit -lt (Get-Date)) {
            $Struct.ReturnString = "CRITICAL: Last successful update at $($Struct.LastSuccesTime.ToString('yyyy/MM/dd HH:mm:ss')) exceeded critical threshold of $($Struct.DaysBeforeCritical) days. "
            $Struct.Exitcode = 2
        }
        elseif($Struct.NumberOfUpdates -ge 1 -and $WarningLimit -lt (Get-Date)) {
            $Struct.ReturnString = "WARNING: Last successful update at $($Struct.LastSuccesTime.ToString('yyyy/MM/dd HH:mm:ss')) exceeded warning threshold of $($Struct.DaysBeforeWarning) days. "
            $Struct.Exitcode = 1
        }
        elseif ($RebootRequired) {
            $Struct.ReturnString = "WARNING: Reboot required. Last successful update: $($Struct.LastSuccesTime.ToString('yyyy/MM/dd HH:mm:ss')). "
            $Struct.Exitcode = 1
        }
        else {
            $Struct.ReturnString = "OK: Last successful update: $($Struct.LastSuccesTime.ToString('yyyy/MM/dd HH:mm:ss')). "
            $Struct.Exitcode = 0
        }
    }
    Catch {
        Write-Log Verbose Info "Something went wrong while processing the output."
    }
...
ElseIf ($Struct.Method -eq 'Custom') {
    Search-WithCustom
}
Else {
