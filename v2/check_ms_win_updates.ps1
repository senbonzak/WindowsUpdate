...
Function Search-WithCustom {
    Write-Log Verbose Info "Querying WSUS updates with WMI method."
    $Struct.ReturnString =''

    Try {
        # Obtention de la date de la dernière mise à jour via WMI
    ...
    Catch { 
        ...
        # Obtention du nombre de mises à jours disponible via PsWindowsUpdate
    Try {
        $Struct.NumberOfUpdates = ((Get-WUList -Notcategory 'Driver' -WarningAction SilentlyContinue) | Measure-Object).count
    }
    Catch {
        ...
        # Vérification de la nécessité d'un redémarrage
    Try {
        $RebootRequired = Get-WURebootStatus -Silent
    }
    Catch {
